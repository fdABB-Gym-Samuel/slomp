...
from ..rooms_helpers import build_room_out
...
async def room_ws(websocket: WebSocket, code: str) -> None:
    ...
    previous = await state.hub.add(code, user_id, websocket)
    ...
    try:
        ...
        logger.exception("ws handler crashed for room=%s user=%s", code, user_id)
    finally:
        await state.hub.remove(code, user_id, websocket)
        async with db.pool().acquire() as conn:
            await conn.execute(
